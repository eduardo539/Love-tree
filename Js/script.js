  ...
  .then((svgText) => {
    ...
    setTimeout(() => {
      allPaths.forEach((path, i) => {
        path.style.transition = `stroke-dashoffset 1.2s cubic-bezier(.77,0,.18,1) ${
          i * 0.08
        }s, fill-opacity 0.5s ${0.9 + i * 0.08}s`;
...
function startFloatingObjects() {
  ...
  function spawn() {
    ...
    setTimeout(() => {
      ...
      el.style.transform = `translate(${drift}px, -110vh) scale(${
        0.8 + Math.random() * 0.6
...
async function showCountdown() {
  ...
  // -------------------------------
  // TIEMPO JUNTO A TI (AÑOS / MESES / DÍAS)
  // -------------------------------
  ...
  const daysToAnniversary = Math.ceil(
    (anniversary - now) / (1000 * 60 * 60 * 24)
  );

  // -------------------------------
  // FECHA INICIO FORMATO LARGO
  // -------------------------------
  const startDateText = startDate.toLocaleDateString("es-MX", {
    day: "numeric",
    month: "long",
    year: "numeric",
  });
  ...
  // -------------------------------
  container.innerHTML = `
    ❤️ <b>${daysTogether}</b> días juntos<br>

    ⏳ <b>Tiempo junto a ti:</b>${years} años, ${months} meses y ${days} días<br>

    📅 Inicio: <b>${startDateText}</b><br><br>
    🎉 Faltan <b>${daysToAnniversary}</b> días para nuestro aniversario
  `;
...
function playBackgroundMusic() {
  ...
  // Crear botón de control si no existe
  ...
  // Toggle reproducción al hacer click
  btn.onclick = () => {
    if (audio.paused) {
      audio.play();
      btn.textContent = "🔊 Música";
